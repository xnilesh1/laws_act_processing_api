...
def pdf_processor_subprocess(pdf_path: str, params: Dict, result_file: str):
    ...
    try:
        ...
        # Overwrite source with original filename for citation
        if original_filename:
            for page in docs:
                page.metadata['document_name'] = original_filename
...
def process_laws(link):
    ...
    try:
        ...
        with safe_pdf_download(link) as (pdf_path, original_filename):
            ...
            logger.info(f"Starting PDF processing subprocess for namespace: caseone-laws")
            process.start()
